...
public class ViewHandlerImpl2 extends ViewHandler {
    ...
    public void doHandler() {
        ...
        if(content != null){
            ...
            bottom.addView(content , vl);
            initContentByIndex(context , jsonObject.getJSONObject("main") , 0 , server , content);
        }

        if(bars != null){
            NavigationBarUtil.getInstance().init(context , this.jsonObject.getJSONObject("main") , server , center , content);
        }

        if(pmd != null){
            bottomPmd.addView(pmd , new RelativeLayout.LayoutParams(context.getResources().getDimensionPixelOffset(R.dimen.bottom_pmd_width) , context.getResources().getDimensionPixelOffset(R.dimen.bottom_pmd_height)));
//            pmd.setBackgroundColor(context.getResources().getColor(R.color.txt));
